...
namespace Microsoft.Samples.Kinect.FaceBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        /// <summary>
        /// COLOR STUFF Reader for color frames
        /// </summary>
        ...
        /// x - axis to Arduino
        /// </summary>
        private SerialPort x_mov;
        /*
        /// <summary>
        /// my port to Arduino
        /// </summary>
        private SerialPort y_mov1;

        /// <summary>
        /// my port to Arduino
        /// </summary>
        private SerialPort y_mov2;

        /// <summary>
        /// my port to Arduino
        /// </summary>
        private SerialPort light;
        */
        /// <summary>
        /// reset  settings
        /// </summary>
        ...
        {
            // port settings
            this.x_mov = new SerialPort();
            this.x_mov.BaudRate = 9600;
            this.x_mov.PortName = "COM15";
            ...
            if (this.reset == true)
            {
                ...
                x_mov.Write(Convert.ToString('s'));
                x_mov.Write(Convert.ToString(90));
                x_mov.Write(Convert.ToString(90));
                x_mov.Write(Convert.ToString(90));
                x_mov.Write(Convert.ToString(0));
                reset = false;
            }
                            ...
                            if (this.faceFrameSources[i].IsTrackingIdValid)
                            {
                                ...
                                if (this.faceFrameResults[i] != null)
                                {
                                   int z = 1;
                                    ...
                                    this.degree = convert_degrees(i);
                                    sendPort(this.x_mov, this.degree);
                                    //sendPort(this.port, convert_degrees(i));
                                    if (!drawFaceResult)
                                    {
                                        drawFaceResult = true;
                                    }
        ...
        /// <summary>
        //Convert positions to degrees and sends to motor
        /// </summary>
        public int convert_degrees(int faceIndex)
        {
            Body body = this.bodies[faceIndex];
            var headJoint = body.Joints[JointType.Head].Position;
            //-0.XXXXXXXXXX to 0.XXXXXXXXX
            float x_mov = headJoint.X * 1000; // convert to decimal -800 to 800 range         

            x_mov = (int)(x_mov / (800 / 90) + 90);
            if(x_mov < 0)
            {
                x_mov = 0;
            }
         ...
         //   Debug.WriteLine("x_mov " + (int)x_mov);
            return (int)x_mov;
        }
        ...
        //sends to motor
        /// </summary>
        public void sendPort(SerialPort port, int x)
        {   //36 good,40
          
            if (stepper == 36)
            {
                this.x_mov.Write(Convert.ToString('s'));
                this.x_mov.Write(Convert.ToString(x));
                this.x_mov.Write(Convert.ToString(dec_test--));
                this.x_mov.Write(Convert.ToString(-1*(dec_test--)));
                this.x_mov.Write(Convert.ToString(!OnOff));
                Debug.WriteLine("degree " + Convert.ToString(x));
